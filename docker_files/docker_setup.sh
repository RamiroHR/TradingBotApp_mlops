...
docker image build . -t ramirohr/tradingbot:1.0.5 -f docker_files/tradingbot_api/Dockerfile
...
## build image
docker image build . -t ramirohr/credentials_api:1.0.1 -f docker_files/users_credentials_api/Dockerfile

## run a container
docker container run --name credential_api -p 7000:7000 -d ramirohr/credentials_api:1.0.1 